...
if __name__ == "__main__":
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        if photo_nums[0] <= 500:
            print("The number of training sets is less than 500, which belongs to a small amount of data. Please pay "
                  "attention to setting a larger training generation (Epoch) to meet sufficient gradient descent "
                  "times (Step).")

        if np.sum(nums) == 0:
            print(
                "I didn't get any target in the data set, please pay attention to modifying the classes_path to "
                "correspond to your own data set, and make sure the label name is correct, otherwise the training "
                "will have no effect!")
